...
files = []
for file in glob.glob("../../WaveFunctions/Ames_test/*_Ames_wavefunction.npy"):
    files.append(file)
...
wavefunction_list = []
for file in files:
    pre_ID = re.findall('../../WaveFunctions/Ames_test/(.*)_Ames_wavefunction.npy', file)
    ID = pre_ID[0]
...
print_data_sets.to_csv("TestAmes_{}samples.csv".format(data_num),index=False)